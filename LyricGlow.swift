// LyricGlow: a floating, glowing lyrics window for Music.app.
...
//
// Run: ./LyricGlow --url http://127.0.0.1:7331 [--all] [--fill] [--scale 1.2]
...
final class LyricWindow: NSObject, WKScriptMessageHandler, NSWindowDelegate {
  ...
  init(app: LyricGlowApp, screenIndex: Int, fill: Bool) {
    self.app = app
    ...
    super.init()
    content.add(self, name: "lyricglow")
    configure()
    load()
  }
  ...
  func pushButtonState() {
    let js = "window.lyricglow && window.lyricglow.markButtons({fullscreen: \(fill), pin: \(pinned), glass: \(glass)})"
...
final class LyricGlowApp: NSObject, NSApplicationDelegate, NSMenuDelegate {
  var options: Options
  ...
  func launchServer() {
    ...
    let support = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
      .appendingPathComponent("LyricGlow", isDirectory: true)
    try? FileManager.default.createDirectory(at: support, withIntermediateDirectories: true)
    let process = Process()
    process.executableURL = URL(fileURLWithPath: python)
    process.arguments = [script, "--port", String(URL(string: options.url)?.port ?? 7331)]
    process.environment = ProcessInfo.processInfo.environment.merging(
      ["LYRICGLOW_CACHE": support.appendingPathComponent("cache").path]) { $1 }
    let handle = try? FileHandle(forWritingTo: logFile(in: support))
    handle?.seekToEndOfFile()
    process.standardOutput = handle
    process.standardError = process.standardOutput
    do { try process.run(); serverProcess = process } catch { NSLog("LyricGlow: could not start server: \(error)") }
  }
  ...
  func buildMainMenu() {
    ...
    appMenu.addItem(withTitle: "Quit LyricGlow", action: #selector(quit), keyEquivalent: "q").target = self
  ...
  func menuNeedsUpdate(_ menu: NSMenu) {
    ...
    menu.addItem(.separator())
    menu.addItem(withTitle: "Quit LyricGlow", action: #selector(quit), keyEquivalent: "q").target = self
  }
  ...
  @objc func showControls() {
    windows.forEach { $0.webView.evaluateJavaScript("window.lyricglow && window.lyricglow.reveal()") }
  }
